...
async function listServices(filters) {
    const { category, parish, minPrice, maxPrice, search, sortBy = 'createdAt', sortOrder = 'desc', page = 1, limit = 20, } = filters;
    const where = {
        deletedAt: null,
        isActive: true,
    };
    if (category) {
        ...
        if (uuidRegex.test(category)) {
            ...
            const cat = await database_1.prisma.category.findUnique({ where: { slug: category } });
            if (cat) {
    ...
    if (minPrice !== undefined || maxPrice !== undefined) {
        ...
        if (minPrice !== undefined)
            where.priceMin.gte = minPrice;
        if (maxPrice !== undefined)
            where.priceMin.lte = maxPrice;
    }
    if (search) {
        where.OR = [
...
async function createService(providerId, data) {
    ...
    if (!category) {
        throw new error_handler_1.AppError('NOT_FOUND', 404, 'Category not found');
    }
...
async function updateService(id, userId, data) {
    const service = await database_1.prisma.service.findFirst({
        where: { id, deletedAt: null },
    });
    if (!service) {
...
async function getNearby(lat, lng, radiusKm = 25, limit = 20) {
    // Approximate lat/lng degree ranges for pre-filtering
    const latDelta = radiusKm / 111.0;
    const lngDelta = radiusKm / (111.0 * Math.cos((lat * Math.PI) / 180));
    const services = await database_1.prisma.$queryRaw `
    SELECT
      s.id,
      s.title,
      s.description,
